...
class ProductRepositoryHelper
{
    ...
    public static function search(
        ...
        foreach ($criteria->getFields() as $field) {
            if (str_starts_with($field, 'children.')) {
                $childrenFields[] = substr($field, strlen('children.'));
                continue;
            }
        ...
        foreach ($parents->getEntities() as $parent) {
            $parent->set('children', new EntityCollection());
        }
        ...
        foreach ($childProducts as $childProduct) {
            $parentId = $childProduct->get('parentId');
            $parent = $parents->get($parentId ?? '');
            if (!$parent) {
                continue;
            }

            $parent->get('children')?->add($childProduct);
        }
